...
//--------------------------------------------------------------
void ofApp::setup(){
    InstrumentObject drum(0);
    shared_ptr<SoundObject> s1(new BassDrumObj());
    drum.add(s1, 36);
    
    shared_ptr<SoundObject> s2(new SnareDrumObj());
    drum.add(s2, 38);
    
    shared_ptr<SoundObject> s3(new HatCloseObj());
    drum.add(s3, 42);
...
void ofApp::update(){
    ...
    cam.setPosition(
        400. * sin(ofGetElapsedTimef()), 0, 400. * cos(ofGetElapsedTimef())
    );
